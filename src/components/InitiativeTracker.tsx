...
import { faPlus, faDiceD20, faTrash, faHeart, faHeartBroken, faRedo, faSortAmountDown } from '@fortawesome/free-solid-svg-icons'
...
export default function InitiativeTracker() {
  ...
  function rerollSingle(id: number) {
    setCombatants(prev =>
      prev.map(c =>
        c.id === id ? { ...c, initiative: roll(20) + c.bonus } : c
  ...
  function heal(id: number, amount: number) {
    setCombatants(prev =>
      prev.map(c =>
    ...
    <div className="initiative-tracker">
      <div className="d-flex align-items-center gap-3 flex-wrap mb-3">
        ...
        {combatants.length > 0 && (
          <>
            ...
            {rolled && (
              <button className="dnd-btn-sm-outline" onClick={() => setRolled(false)} title="Permitir retirar">
                <FontAwesomeIcon icon={faRedo} style={{ marginRight: '4px' }} />
                Reiniciar tiradas
              </button>
            )}
            <button className="dnd-btn-sm-outline" onClick={clearAll} style={{ borderColor: 'rgba(255,0,0,0.4)' }}>
      ...
      {combatants.length === 0 && (
        <p className="textcont" style={{ marginTop: '1rem', opacity: 0.6 }}>
          Añade combatientes y tira iniciativa para comenzar.
        </p>
      )}
                ...
                <div className={`init-row ${idx === 0 && rolled ? 'init-current' : ''}`}>
                  ...
                  <div className="init-info">
                    ...
                    <span className="init-stats">
                      CA {c.ac} &middot; PG {c.hp}/{c.maxHp}
                    </span>
                  </div>
                  ...
                  <div className="init-actions">
                    <div className="init-hp-actions">
                      <button
                        className="init-btn-hp"
                        onClick={() => {
                          const amt = prompt('Daño:') || '0'
                          damage(c.id, Math.max(0, parseInt(amt) || 0))
                        }}
                        title="Daño"
                      >
                      ...
                      <button
                        className="init-btn-hp init-heal"
                        onClick={() => {
                          const amt = prompt('Curación:') || '0'
                          heal(c.id, Math.max(0, parseInt(amt) || 0))
                        }}
                        title="Curación"
                      >
                    ...
                    {rolled && (
                      <button
                        ...
                        onClick={() => rerollSingle(c.id)}
                        title="Retirar iniciativa"
                      >
